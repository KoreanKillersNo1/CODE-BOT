...
async def on_message(message, checkUser=None, DeleteAccount=None, Signup=None, checkUserNum=None, userInfo=None,getRank=None, chekcUserNum=None, getMoney=None, remit=None, modifyMoney=None, addLoss=None,cur_money=None):
    ...
    if message.content.startswith("!마크"):
      embed = discord.Embed(title="ㅤ", description="마크 무료로 설치하는법 어떤사람이 나 3만원 주고 마크 설치함 라고 하길래 알려줬다 1. 마크 사이트를 들어간다 2. 다운로드를 누른다 3. 런처 다운받는다 4. 설치한다 5. 로그인을 한다 6. 재밌게 플레이를 한다 3만원에 설치한 사람은 호구다", color=0x000000)
      await message.channel.send(embed=embed)
    ...
    if message.content.startswith("!송금"):
      senderExistance, senderRow = checkUser(message.author.name, message.author.id)
      receiverExistance, receiverRow = checkUser(user.name, user.id)

      print("송금하려는 돈: ", money)

      s_money = getMoney(message.author.name, senderRow)
      r_money = getMoney(user.name, receiverRow)
      ...
      if s_money >= int(money) and int(money) != 0:
        ...
        remit(message.author.name, senderRow, user.name, receiverRow, money)

        print("송금이 완료되었습니다. 결과를 전송합니다.")

        embed = discord.Embed(title="송금 완료", description="송금된 돈: " + money, color=0x77ff00)
        embed.add_field(name="보낸 사람: " + message.author.name,value="현재 자산: " + str(getMoney(message.author.name, senderRow)))
        embed.add_field(name="→", value=":moneybag:")
        embed.add_field(name="받은 사람: " + user.name, value="현재 자산: " + str(getMoney(user.name, receiverRow)))
        ...
        await message.channel.send("0원을 보낼 필요는 없죠")
      else:
        print("돈이 충분하지 않습니다.")
        print("송금하려는 돈: ", money)
        print("현재 자산: ", s_money)
        await message.channel.send("돈이 충분하지 않습니다. 현재 자산: " + str(s_money))
